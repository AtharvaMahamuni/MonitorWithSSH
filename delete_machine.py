...
if __name__ == "__main__":
    ...
    if os.path.isfile(machine_list_file):
        try:
            ...
            machine_id = int(input("Choose machine_id to delete: "))
            
            double_check = str(input("Are you sure you want to delete machine with id [{}]?(yes/no)".format(machine_id)))
            if double_check.lower() == "yes":
                query = "DELETE FROM machine_list WHERE machine_id={};".format(machine_id)
                
                print("Executing deletion query for machine_id [{}]...".format(machine_id))
                machine_list_cursor.execute(query)
            ...
            if machine_list_file_connection:
                ...
                print('Changes completed in database...')
